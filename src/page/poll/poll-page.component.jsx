...
import {isAnsweredByCurrentUser, selectCurrentPoll, selectIsIdValid} from '../../redux/poll/poll.selectors';
         ...
         <div className ="results">
            ...
            {mappable(poll.answers).map(answer => 
                <div key={answer.name} className="poll poll__result">
                    <p className="row j-between"><span className="answer">{answer.name} </span> 
                    <span className="percentage">{ (answer.number/pollsize) * 100 }%</span></p>
                    <div className="bar-holder">
                        <div className="bar-line" style={{width:`${answer.number/pollsize * 100}%`}}></div>
                    </div>
                </div>
            )}
            <p className="text-center my-4"><small>You answered <span className="user-answer">{poll.users_answered[currentUser].answer}</span></small></p>
        </div> 
